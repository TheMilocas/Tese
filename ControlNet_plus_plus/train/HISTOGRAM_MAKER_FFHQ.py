...
# -------- CONFIG --------
EMBEDDING_PREFIX = "../../"
embedding_dir_original = "./DATASETS_FOR_TESTING/FFHQ/input_embeddings"
embedding_dir_base = "./TEST_FFHQ/embeddings_base"
embedding_dir_controlnet = "./TEST_FFHQ/embeddings_controlnet"  
NUM_SAMPLES = 2953 
SAVE_PREFIX = "ffhq"
# ------------------------
...
for i in tqdm(common_ids, desc="Computing similarities"):
    ...
    gt = torch.from_numpy(np.load(os.path.join(embedding_dir_original, f"{i:03d}.npy"))).unsqueeze(0)
    emb_base = torch.from_numpy(np.load(os.path.join(embedding_dir_base, f"{i:03d}.npy"))).unsqueeze(0)
    emb_cn   = torch.from_numpy(np.load(os.path.join(embedding_dir_controlnet, f"{i:03d}.npy"))).unsqueeze(0)
...
plt.legend()
plt.tight_layout()
plt.savefig(f"TEST_FFHQ/similarity_hist_{SAVE_PREFIX}.png", dpi=300)
...
plt.tight_layout()
plt.savefig(f"TEST_FFHQ/similarity_delta_hist_{SAVE_PREFIX}.png", dpi=300)
